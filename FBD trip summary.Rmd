...
library(rgdal);library(dplyr);library(ggplot2);library(ggplot2);library(sf);library(tidyverse)

year<-2021
YYYY_MM<-'2021_01'
loc_base<-"Doubtful Sound/Deep Cove"
tripdate_s<-"25"
tripdate_e<-"29 Jan 2021"
team<-c("Dave Johnson, DOC","Anja Köhler, DOC","Max Harvey, University of Otago")
vessel<-"Nemo"
known_calves<-c()
new_calves<-c("Mako (Whitetip)", "Shivers (Scratchy)", "Sux (Five)", "Vortex (Spiral)", "Yule (Glob)")
pop_est<-"2019: 68 (95% CI = 67–69)"

wx_comments<-""
calf_comments<-"The timing of the Doubtful Sound summer trip was adjusted this year to attempt to better fit with the peak calving time of bottlenose dolphins. Potentially five new calves were observed this season, the largest number of new calves since 2016. All mothers of newborns have had previous calves which have survived to >3 years old. A new individual was also ID’d in the population approximately 1 year old, observed swimming with Bafta. This is the second time this individual was observed with Bafta, potentially indicating Bafta is a new mother from the 2019/2020 breeding season."
nextsteps_comments<-"Unfortunately dolphins were only sighted on two days during this survey. It is therefore necessary to confirm if the new calves identified are correct and assigned to the correct mother during the winter trip – if they survive."
...
disco<-function(x){
  ...
  desc.df<-desc.df%>%
    mutate(date = lubridate::dmy(names(freqcap)))%>%
...
ggplot(disco_data, aes(x=date, y = disc))+
  ...
  ylab("Number of identified individuals")+
...
unseen_two_years<-all_first_last%>%
  ...
  filter(last_date < max(as.Date(tracks_df$time)) - lubridate::years(1) & last_date >= max(as.Date(tracks_df$time)) - lubridate::years(2))%>%
...
\vspace*{-1mm}
\rule[\baselineskip]{\linewidth}{0.4pt}\vspace{-6mm}
Effort (nautical miles): \hfill `r round(sum(tracks_df$dist_km, na.rm=TRUE),0)`
\vspace*{-1mm}
\rule[\baselineskip]{\linewidth}{0.4pt}\vspace{-6mm}
...
```
\vspace*{-10mm}
Fig. 1. Dolphin sighting locations are indicated by the red diamonds, and included: `r stringi::stri_replace_last(do.call("paste", c(as.list(unique(sig$LOCATION)), sep = ", ")), fixed = ",", ", and")`
